...
function code_coverage_report {
  ...
  if [[ "$CODACY" == "true" ]]; then
    (cd "$target_ws" && curl -Ls -o codacy-coverage-reporter "$(curl -Ls https://api.github.com/repos/codacy/codacy-coverage-reporter/releases/latest | jq -r '.assets | map({name, browser_download_url} | select(.name | contains("codacy-coverage-reporter-linux"))) | .[0].browser_download_url')") 
  ...
  if [[ "$CODECOV" == "true" ]]; then
    (cd "$target_ws" && curl -s https://codecov.io/bash > .codecov)
